...
export default function PackagesSection() {
  return (
    <motion.section id="packages" className="py-20 px-10 bg-gradient-to-b from-white to-gray-50" variants={itemVariants}>
      <h2 className="text-4xl font-bold mb-4 text-purple-600 text-center">Wedding Packages</h2>
      ...
      <motion.ul className="grid md:grid-cols-2 lg:grid-cols-3 gap-10" variants={containerVariants}>
        {packages.map((pkg, index) => (
          <motion.li key={index} className="bg-white p-8 rounded-2xl shadow-lg border border-gray-200 hover:border-purple-500 hover:shadow-2xl transform hover:scale-105 transition-all duration-300 flex flex-col items-center text-center group" variants={itemVariants}>
            ...
            <span className="inline-block px-6 py-2 text-2xl font-bold text-purple-600 bg-purple-50 rounded-full">{pkg.price}</span>
          </motion.li>
        ))}
      </motion.ul>
    </motion.section>
